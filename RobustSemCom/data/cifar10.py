...
class CIFAR10(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Ensure `img` is a Tensor before adding noise
        if not isinstance(img, torch.Tensor):
            raise TypeError(f"Expected img to be a Tensor, but got {type(img)}")
...
def get_cifar10_transform(cfg):
    ...
    train_transform = v2.Compose(
        [
            v2.ToImageTensor(),  # Convert PIL image to Tensor (MUST come first)
            v2.ToDtype(torch.float32),  # Ensure correct dtype
