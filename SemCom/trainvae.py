...
from modelvae import DeepJSCC, ratio2filtersize
...
def train_epoch(model, optimizer, param, data_loader):
    ...
    for iter, (images, _) in enumerate(data_loader):
        ...
        # Gọi model.forward() để nhận thêm mu và logvar
        outputs, mu, logvar = model.forward(images)

        # Truyền đầy đủ các tham số vào loss function
        loss = model.loss(images, outputs, mu, logvar)
...
def evaluate_epoch(model, param, data_loader):
    ...
    with torch.no_grad():
        for iter, (images, _) in enumerate(data_loader):
            ...
            # Gọi model.forward() và lấy giá trị x_hat
            outputs, mu, logvar = model.forward(images)
            
            # Chỉ áp dụng normalization lên x_hat
            outputs = image_normalization('denormalization')(outputs)
            images = image_normalization('denormalization')(images)

            # Tính loss với các tham số đầy đủ
            loss = model.loss(images, outputs, mu, logvar)
            epoch_loss += loss.item()
        epoch_loss /= (iter + 1)
...
def config_parser_pipeline():
    ...
    parser.add_argument('--ratio_list', default=['1/12'], nargs='+', help='ratio_list')
    parser.add_argument('--channel', default='AWGN', type=str,
                        choices=['AWGN', 'Rayleigh'], help='channel')
    parser.add_argument('--dataset', default='custom', type=str, choices=['cifar10', 'imagenet', 'custom'], help='dataset')  ## to update the customm dataset
...
def train_pipeline(params):
    ...
    if dataset_name == 'cifar10':
        ...
        train_loader = DataLoader(train_dataset, shuffle=True, batch_size=params['batch_size'], num_workers=params['num_workers'])
        test_dataset = CustomDataset(root_dir='./dataset/test', transform=transform)
        test_loader = DataLoader(test_dataset, shuffle=True, batch_size=params['batch_size'], num_workers=params['num_workers'])
...
def config_parser():  # deprecated
    ...
    parser.add_argument('--snr_list', default=['19'], nargs='+', help='snr_list')
    parser.add_argument('--ratio_list', default=['1/12'], nargs='+', help='ratio_list')
